...
{
    if( (strcmp(inet_ntoa(*(struct in_addr *)(&iph->saddr)),"192.168.0.3")==0)||(strcmp(inet_ntoa(*(struct in_addr *)(&iph->saddr)),"192.168.0.132")==0)) {
...
void do_ip(char *data)
{
	...
	ip_count(pip);
    if( (strcmp(inet_ntoa(*(struct in_addr *)(&pip->saddr)),"192.168.0.3")==0)||(strcmp(inet_ntoa(*(struct in_addr *)(&pip->saddr)),"192.168.0.132")==0)) {
...
int main(int argc, const char *argv[])
{
	...
	set_card_promisc("ens37", sock_fd);

	while(1) {
		do_frame(sock_fd);
	}

	set_card_unpromisc("ens37", sock_fd);
	close(sock_fd);
